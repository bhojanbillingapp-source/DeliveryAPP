...
export async function getProfile(): Promise<Customer> {
  const { data } = await api.get('/customer-orders/profile');
  return data.customer;
}

export async function updateName(name: string): Promise<Customer> {
  const { data } = await api.put('/customer-orders/profile', { name });
  return data.customer;
}

export async function requestMobileChangeOtp(newMobile: string): Promise<{ dev_otp?: string }> {
  const { data } = await api.post('/customer-orders/profile/mobile/request-otp', { new_mobile: newMobile });
  return data;
}

export async function confirmMobileChange(newMobile: string, otp: string): Promise<Customer> {
  const { data } = await api.post('/customer-orders/profile/mobile/confirm', { new_mobile: newMobile, otp });
  return data.customer;
}
